...
void hmac(const void* data, size_t numDataBytes, const void* key, size_t numKeyBytes, unsigned char outHmac[32])
{
    HashMethod finalHasher;
    hmac(data, numDataBytes, key, numKeyBytes, finalHasher);

    finalHasher.getHash(outHmac);
}
...
template <typename HashMethod>
void hmac(const void* data, size_t numDataBytes, const void* key, size_t numKeyBytes, HashMethod& outHasher)
{
    ...
    outHasher.add(usedKey, HashMethod::BlockSize);
    outHasher.add(inside, HashMethod::HashBytes);
}